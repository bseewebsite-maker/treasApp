      ...
      <main className="flex-1 overflow-y-auto p-4">
        <div className="bg-white rounded-lg shadow">
            <ul className="divide-y divide-gray-200">
                <li onClick={() => setIsConfirmOpen(true)} className="p-4 flex justify-between items-center cursor-pointer hover:bg-red-50">
                    <div className="flex items-center">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">Permanently delete all collections, students, and settings.</p>
                        </div>
                    </div>
                </li>
            </ul>
      ...
      {isConfirmOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-6">
                ...
                <p className="mt-2 text-sm text-gray-600">This will permanently delete <strong>all data</strong> from the app, including students, collections, and your profile. This action cannot be undone.</p>
                <div className="mt-6 flex justify-end space-x-3">
                    <button onClick={() => setIsConfirmOpen(false)} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Cancel</button>
                    <button onClick={handleClearAllData} className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700">Yes, Clear Everything</button>
                </div>
            </div>
        </div>
      )}
...
const AppContent: React.FC = () => {
  ...
  if (isAddingCollection) {
    return (
      <AddCollectionScreen
        onBack={() => {
            // When going back, if the current state is the 'addCollection' view,
            // use history.back() to pop the state and trigger the popstate listener.
            if (window.history.state && window.history.state.view === 'addCollection') {
                window.history.back();
            } else {
                // Otherwise, just update the state directly. This handles cases where history might be out of sync.
                setIsAddingCollection(false);
            }
        }}
