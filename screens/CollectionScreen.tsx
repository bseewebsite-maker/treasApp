...
const CollectionCard: React.FC<{
  ...
  return (
    <div
      className={`relative bg-white rounded-xl shadow-md overflow-hidden mb-5 prevent-select transition-shadow, transition-transform ${isSelected ? 'ring-2 ring-blue-500 scale-105 shadow-xl' : 'transform hover:scale-105 hover:shadow-lg'}`}
    >
      <div 
        ref={actionsRef}
      ...
      >
        ...
        <button
          ...
          className="w-1/2 h-full flex flex-col items-center justify-center text-white font-semibold bg-green-500 hover:bg-green-600 transition-colors"
      ...
      >
        <div className="p-5">
          <div className="flex justify-between items-start">
            <div>
              {collection.type === 'ulikdanay' ? (
                <h3 className="text-lg text-gray-800">
                  <strong className="font-bold uppercase">{collection.name.match(/Month of (\w+)/)?.[1] || 'Ulikdanay'}</strong>
                  <span className="font-normal ml-2">Ulikdanay</span>
                </h3>
              ) : (
                <h3 className="font-semibold text-lg text-gray-800">{collection.name}</h3>
              )}
              <p className="text-xs text-gray-500 mt-1">
                <span>{collection.deadline ? `Deadline: ${new Date(collection.deadline).toLocaleDateString()}` : 'No Deadline'}</span>
                {collection.targetAmount && collection.targetAmount > 0 && (
                  <>
                    <span className="mx-1.5 font-bold text-gray-400">&middot;</span>
                    <span>₱{collection.targetAmount.toLocaleString()} per student</span>
                  </>
                )}
              </p>
            </div>
          ...
          <div className="mt-5">
            <div className="flex justify-between items-end">
              ...
              <div className="text-right">
                <p className="font-semibold text-gray-700">{`${paidStudentsCount} / ${numStudentsInCollection}`}</p>
                <p className="text-xs text-gray-500">students paid</p>
              </div>
            </div>
          </div>
...
const CollectionScreen: React.FC<CollectionScreenProps> = ({ onSelectCollection, onAddCollection }) => {
  ...
  const handleExport = (collection: Collection) => {
    ...
    if (hasSetAmount) {
        const amountPerStudentCell = 'B3';
        if (worksheet[amountPerStudentCell]) {
            worksheet[amountPerStudentCell].t = 'n';
            worksheet[amountPerStudentCell].z = '"₱"#,##0.00';
        }
    }

    const amountPaidColIndex = 2 + customFieldHeaders.length + (hasSetAmount ? 0 : 1);

    studentData.forEach((_row, index) => {
        if(!hasSetAmount) {
            const toPayCellAddress = XLSX.utils.encode_cell({c: amountPaidColIndex - 1, r: bodyStartRow + index});
            if(worksheet[toPayCellAddress] && typeof worksheet[toPayCellAddress].v === 'number') {
                worksheet[toPayCellAddress].t = 'n';
                worksheet[toPayCellAddress].z = '"₱"#,##0.00';
            }
        }
        const paidCellAddress = XLSX.utils.encode_cell({c: amountPaidColIndex, r: bodyStartRow + index});
        if(worksheet[paidCellAddress] && typeof worksheet[paidCellAddress].v === 'number') {
            worksheet[paidCellAddress].t = 'n';
            worksheet[paidCellAddress].z = '"₱"#,##0.00';
        }
    });

    const customColsWidths = customFieldHeaders.map(() => ({ wch: 20 }));
    const colWidths = [{wch: 15}, {wch: 30}, ...customColsWidths];
    if(!hasSetAmount) {
        colWidths.push({wch:15});
    }
    colWidths.push({wch: 15}, {wch: 20}, {wch: 12}, {wch: 12});
    ...
    if (collection.type === 'ulikdanay') {
      const monthMatch = collection.name.match(/Month of (\w+)/);
      if (monthMatch && monthMatch[1]) {
        const month = monthMatch[1];
        fileName = `${month} - Ulikdanay Fund.xlsx`;
      } else {
        fileName = `${collection.name.replace(/[^\w\s-]/g, '').replace(/[\s-]+/g, '_')}_Export.xlsx`;
      }
    } else {
      fileName = `${collection.name.replace(/[^\w\s-]/g, '').replace(/[\s-]+/g, '_')}_Export.xlsx`;
    }
  ...
  return (
    <div className="p-4 pb-32">
      ...
      <p className="text-sm text-gray-500 mb-6 truncate">
        {selectedIds.length > 0 ? `${selectedIds.length} selected` : 'Swipe a card for actions, or long-press to select.'}
      </p>
      ...
      {collections.length > 0 ? (
        ...
        <div className="text-center py-20 px-4">
            <div className="flex justify-center items-center mb-4">
                <div className="bg-gray-200 rounded-full p-4">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 7.125A2.25 2.25 0 0 1 4.5 4.875h15A2.25 2.25 0 0 1 21.75 7.125v1.5625a2.25 2.25 0 0 1-2.25 2.25H4.5A2.25 2.25 0 0 1 2.25 8.6875V7.125ZM3.75 14.25a2.25 2.25 0 0 0 2.25-2.25h12a2.25 2.25 0 0 0 2.25 2.25v2.25a2.25 2.25 0 0 1-2.25 2.25h-12a2.25 2.25 0 0 1-2.25-2.25v-2.25Z" />
                    </svg>
                </div>
            </div>
            <h3 className="text-lg font-semibold text-gray-700">No collections yet</h3>
            <p className="text-gray-500 mt-2">Tap the '+' button below to start a new collection.</p>
        </div>
      )}
      ...
      {selectedIds.length > 0 ? (
            ...
            <div className="bg-white rounded-xl shadow-lg p-2 max-w-md mx-auto">
                <div className="flex justify-between items-center gap-2">
                    <button
                        ...
                        className="px-4 py-3 bg-gray-200 text-gray-800 rounded-lg font-semibold w-1/3 text-center transition-colors hover:bg-gray-300 text-sm"
                    >
                        Cancel
                    </button>
                    <button
                        onClick={() => setIsDeleteAllConfirmOpen(true)}
                        className="px-4 py-3 text-red-700 bg-red-100 rounded-lg font-semibold w-1/3 text-center transition-colors hover:bg-red-200 text-sm"
                    >
                        Delete All
                    </button>
                    <button
                        onClick={() => setIsDeleteSelectedConfirmOpen(true)}
                        className="px-4 py-3 bg-red-600 text-white rounded-lg font-semibold w-1/3 text-center transition-colors hover:bg-red-700 text-sm"
                    >
        ...
        <div className="fixed bottom-24 right-6 z-10">
            <button
                ...
                className="bg-blue-500 hover:bg-blue-600 text-white rounded-2xl w-16 h-16 flex items-center justify-center shadow-lg transition-transform transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
      ...
      {isDeleteSelectedConfirmOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-6">
                <h3 className="text-lg font-bold text-gray-900">Delete Selected?</h3>
                <p className="mt-2 text-sm text-gray-600">Are you sure you want to delete <strong>{selectedIds.length} collection(s)</strong>? This action cannot be undone.</p>
                <div className="mt-6 flex justify-end space-x-3">
                    <button onClick={() => setIsDeleteSelectedConfirmOpen(false)} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Cancel</button>
                    <button onClick={handleConfirmDeleteSelected} className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700">Delete</button>
                </div>
            </div>
        </div>
      )}
      ...
      {isDeleteAllConfirmOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-6">
                <h3 className="text-lg font-bold text-gray-900">Delete All Collections?</h3>
                <p className="mt-2 text-sm text-gray-600">Are you sure you want to delete <strong>ALL</strong> collections? This action cannot be undone.</p>
                <div className="mt-6 flex justify-end space-x-3">
                    <button onClick={() => setIsDeleteAllConfirmOpen(false)} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Cancel</button>
                    <button onClick={handleConfirmDeleteAll} className="px-4 py-2 bg-red-800 text-white rounded-md hover:bg-red-900">Delete All</button>
                </div>
            </div>
        </div>
      )}
